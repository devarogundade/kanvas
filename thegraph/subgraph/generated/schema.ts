...
export class GameEvent extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save GameEvent entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type GameEvent must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("GameEvent", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): GameEvent | null {
    return changetype<GameEvent | null>(
      store.get_in_block("GameEvent", id.toHexString())
    );
  }

  static load(id: Bytes): GameEvent | null {
    return changetype<GameEvent | null>(
      store.get("GameEvent", id.toHexString())
    );
  }
  ...
  set playerId(value: Bytes) {
    this.set("playerId", Value.fromBytes(value));
  }
